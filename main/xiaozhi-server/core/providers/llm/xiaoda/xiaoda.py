...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        self.base_url = config.get("base_url", "https://localhost/v1").rstrip("/")
        self.session_conversation_map = {}  # 存储session_id和conversation_id的映射
        check_model_key("XiaoDa", self.api_key)
                        ...
                        if line.startswith(b"data: "):
                            ...
                            if event.get("event") == "workflow_finished":
                                if event["data"]["status"] == "succeeded":
                                    # yield event["data"]["outputs"]["text"]
                                    logger.bind(tag=TAG).info(event["data"]["outputs"]["text"])
                                    return
                                else:
                                    yield "【服务响应异常】"
                elif self.mode == "completion-messages":
...
if __name__ == "__main__":
    config = {
        "api_key": "app-vbtG8HoXATPyEMYdrYs6rPGl",
        "mode": "workflows/run",
        "base_url": "http://116.228.215.13:33303/v1",
    }
    llm = LLMProvider(config)
    for token in llm.response("session_id", [{"role": "user", "content": "hello"}]):
        print(token)